...
<div class="card">
    ...
    <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-1">
                    <button class="btn btn-success">
                        {{ trans('booking.dashboard.search') }}
                    </button>
                </div>
            </div>
        </form>
        @if($rooms !== null)
                        ...
                        <tbody>
                            @foreach($rooms as $room)
                                <tr>
                                    ...
                                    <td>
                                        {{ $room->hourly_rate ? '$' . number_format($room->hourly_rate, 2) : trans("booking.dashboard.free") }}
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#bookRoom" data-room-id="{{ $room->id }}">
                                            {{ trans('booking.dashboard.book_room') }}
                                        </button>
                                    </td>
            ...
            @else
                <p class="text-center">{{ trans('booking.dashboard.no_rooms_available') }}</p>
            @endif
        @endif
    </div>
</div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ trans('booking.dashboard.booking_of_a_room') }}</h5>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.bookRoom') }}" method="POST" id="bookingForm">
                    ...
                    <div class="form-group">
                        <label for="recurring_until">{{ trans('booking.dashboard.recurring_until') }}</label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="submitBooking">{{ trans('booking.dashboard.submit') }}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('booking.dashboard.close') }}</button>
            </div>
        </div>
    </div>
</div>
